...
use glory_backend::services::{commerce_outbox, SessionService};
...
async fn create_user(state: &AppState, role: &str, email_prefix: &str) -> (Uuid, String) {
...
fn test_event(event_id: &str, event_type: &str, object: Value) -> Value {
    json!({
        "id": event_id,
        "type": event_type,
        "livemode": false,
